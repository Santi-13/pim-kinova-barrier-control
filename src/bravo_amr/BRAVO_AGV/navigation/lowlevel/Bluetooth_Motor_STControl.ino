...
void setup() {
  ...
  BT.begin("ESP32_AGV_Control"); // Nombre de su dispositivo Bluetooth y en modo esclavo
...
void loop() {
  ...
  if (BT.available()){
    ...
    switch (incoming){
      ...
      case 67:
        Serial.println("Stop");
        for (int i = standard_speed; i > 10; i--){
          ledcWrite(PWM_Channel0, i); // pulse a signal continually at the rate of 64 
          ledcWrite(PWM_Channel1, i);
          ledcWrite(PWM_Channel2, i);
          ledcWrite(PWM_Channel3, i);
        }
        ledcWrite(PWM_Channel0, 0); // pulse a signal continually at the rate of 64 
        ...
        break;
      case 48:
        Serial.println("Stop Alt");
        break;
      case 0:
        break;
      } 
    }
    if (act_control == 1){supertright(VelReq);}
    if (act_control2 == 1){supertleft(VelReq2);}
    delay(100);
}
